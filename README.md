# DataMind-AI-Virtual-Tutor
Multi-agent AI tutor built with n8n and Claude Sonnet
...
Workflow steps
Student sends a message via the chat trigger
Claude Agent reasons about intent and selects a tool
An IF node checks whether the session is complete
Response is formatted from Markdown to HTML
A task router directs the query to the correct tool node
At end of session, Gmail sends a personalised daily progress report automatically
Status
...
Download AI Virtual Tutor.json from this repo.
In n8n, go to Settings → Import from File (or use the ⋯ menu → Import Workflow on the canvas).
Select the downloaded JSON file.
Reconnect your own credentials for Claude (Anthropic), SerpAPI, OpenAI, and Gmail — credentials are not included in the export for security reasons.
Activate the workflow to generate a live Production URL for the chat trigger.
Roadmap
